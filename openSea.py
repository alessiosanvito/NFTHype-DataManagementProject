...
class OpenSeaBot:
    ...
    def scrapePage(self, page):
        ...
        while len(scraped) != 100: #finchè non cattura 100 records
            ...
            try: # se non ne trova più e solleva un exception, scorre la pagina (react non carica tutti i componenti ma solo quelli visibili)
                i = 1
                while True: #finchè trova records
                    ...
                    if currentNFTtitle not in scraped and currentNFTtitle != "": # se non ha raccolto ancora questo record
                        ...
                        time.sleep(0.25)
                        action.move_to_element(currentNFTownersElement) # metti il cursore sul numero per mostrare la cifra precisa
                ...
                self.webdriver.execute_script("window.scrollTo(0," + str(scroll) + ")")
                scroll += 200
...
while True: #non sappiamo quante pagine ci siano effettivamente, interrompiamo quando abbiamo abbastanza dati 
    ...
    dataframe =  pd.DataFrame(currentNFTS, columns=["Collection", "Volume", "Floor Price", "Owners", "Items"]).reindex(range(index, index + 100)) #crea il dataset

    #legacy = pd.read_json("nft.json", orient="index") #legge il json con quelli già raccolti

    #legacy = legacy.append(dataframe, ignore_index=True) #unisce i nuovi ai vecchi

    #legacy.to_json("nft.json",orient="index") # salva tutto nel file nft
    
    json = dataframe.to_dict(orient = "record")
    
    collection.insert_many(json)

    bot.webdriver.quit() #chiude la pagina web (questo per evitare il consumo eccessivo di memoria)
